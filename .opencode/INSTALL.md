# Install git-workflow skill (OpenCode)
...
- Personal (all projects): target = `~/.config/opencode/skills/git-workflow`
  (older OpenCode versions may use `~/.opencode/skills/git-workflow` — check
  which directory already exists on this machine, or which one OpenCode's own
  docs/config point to, and use that one)
- Project (this repo only): target = `.opencode/skills/git-workflow`
...
- It's active starting now, or next session if OpenCode needs a restart to
  discover new skills — check and say which applies.
